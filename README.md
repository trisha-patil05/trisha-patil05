<div align="center">

<!-- Availability Badge -->
<img src="https://img.shields.io/badge/🟢%20Open%20to%20Internships-MERN%20%7C%20Full--Stack-00D4FF?style=for-the-badge&labelColor=0D1117" />

<br/><br/>

<!-- Typing SVG -->
<img src="https://readme-typing-svg.herokuapp.com?font=JetBrains+Mono&weight=700&size=32&pause=1200&color=00D4FF&center=true&vCenter=true&width=700&lines=Hi+%F0%9F%91%8B%2C+I'm+Trisha+Patil;" />

<br/>

<h3>
💻 React · Node.js · MongoDB · Express &nbsp;|&nbsp; B.Tech IT 2027 · Gujarat
</h3>

<p>
  <a href="https://www.linkedin.com/in/trisha-patil-629ab3300" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?logo=linkedin&logoColor=white&style=for-the-badge" height="28"/>
  </a>
  &nbsp;
  <a href="mailto:trishapatil05@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?logo=gmail&logoColor=white&style=for-the-badge" height="28"/>
  </a>
  &nbsp;
  <a href="https://github.com/trisha-patil05" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-181717?logo=github&logoColor=white&style=for-the-badge" height="28"/>
  </a>
  &nbsp;
  <a href="https://instagram.com/trisha_p_05" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-E4405F?logo=instagram&logoColor=white&style=for-the-badge" height="28"/>
  </a>
</p>

</div>

---

## 👩‍💻 About Me

B.Tech IT student (2027) from Gujarat with hands-on experience building full-stack MERN applications and real production UI. I've shipped features used by real users during my internship at Zymo Research and I'm currently building projects that go beyond basic CRUD.

- 🔭 Currently building: **Assignment Management System v2** with analytics dashboard
- 📖 Learning: **Next.js**, System Design, DSA patterns
- 🎯 Goal: On-campus placement 2027 | Open to internships right now
- 🧩 Interested in: Web Dev, IT Automation, developer tooling

---

## 💼 Experience

### Frontend Developer Intern — [Zymo Research](https://www.zymoresearch.com/)
`HTML` `CSS` `JavaScript` `Flutter` `Dart`

- Built **4+ reusable UI components** integrated across the production web platform, reducing inconsistency in the design system
- Designed and deployed **advertisement banners** directly to the live website
- Contributed to the **Flutter mobile app** — built 2 screens with state management, improving user onboarding flow
- Worked in a professional team environment with real deployment cycles and code reviews

---

## 🚀 Projects

### 🍕 HeatTreat Pizza — Full-Stack Food Ordering App
`React.js` `Node.js` `Express.js` `MongoDB`

A complete MERN-stack pizza delivery app with real user flows — not a tutorial clone.

- User authentication (JWT), pizza customization, cart & order management
- Dynamic pricing engine, clean UI/UX, real-time order status tracking
- Structured backend with RESTful API and MongoDB data modeling

<a href="https://github.com/trisha-patil05/HeatTreat-Pizza">
  <img src="https://img.shields.io/badge/GitHub-Source_Code-181717?logo=github&style=for-the-badge" />
</a>
<!-- Add this when deployed: <a href="YOUR_LIVE_LINK"><img src="https://img.shields.io/badge/Live-Demo-00D4FF?style=for-the-badge" /></a> -->

---

### 📚 Assignment Management System — Role-Based Academic Platform
`React.js` `Node.js` `Express.js` `MongoDB`

A web app that solves a real problem — managing assignments across student and teacher roles.

- Role-based access control (student / teacher), secure login, assignment upload & submission
- Grading system, dashboard views per role, clean submission tracking
- Currently adding **v2 analytics dashboard** with submission trends and grade distributions

<a href="https://github.com/trisha-patil05/assignment-management-system">
  <img src="https://img.shields.io/badge/GitHub-Source_Code-181717?logo=github&style=for-the-badge" />
</a>

---

## 🧠 Tech Stack

| Layer | Technologies |
|---|---|
| **Languages** | JavaScript · Python · Dart |
| **Frontend** | React.js · Next.js · HTML5 · CSS3 · Tailwind CSS |
| **Backend** | Node.js · Express.js |
| **Database** | MongoDB |
| **Mobile** | Flutter |
| **Tools** | Git · GitHub · VS Code · Postman |

---

## 🔢 DSA & Problem Solving

![LeetCode](https://img.shields.io/badge/LeetCode-Practicing-FFA116?style=for-the-badge&logo=leetcode&logoColor=black)
![GFG](https://img.shields.io/badge/GeeksforGeeks-Practicing-2F8D46?style=for-the-badge&logo=geeksforgeeks&logoColor=white)

Practicing consistently on **LeetCode** and **GeeksforGeeks**  
Topics: Arrays · Strings · Recursion · Trees · Graphs · Dynamic Programming  
Solutions repo → [dsa-with-javascript](https://github.com/trisha-patil05/dsa-with-javascript)

---

## 🏆 Certifications

| Certificate | Issuer |
|---|---|
| Python Programming | Scaler Academy |
| Software Development Job Simulation | Datacom via Forage |
| Software Engineering Virtual Experience | Accenture via Forage |
| Responsive Web Design | freeCodeCamp |

---

## 📊 GitHub Stats

<div align="center">
  <img height="175em" src="https://github-readme-stats.vercel.app/api?username=trisha-patil05&show_icons=true&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true" />
  <img height="175em" src="https://streak-stats.demolab.com?user=trisha-patil05&theme=tokyonight&hide_border=true" />
  <br/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=trisha-patil05&layout=compact&theme=tokyonight&hide_border=true" />
</div>

---

<div align="center">
  <i>"Build things that work. Ship things that matter."</i>
  <br/><br/>
  <img src="https://visitor-badge.laobi.icu/badge?page_id=trisha-patil05.trisha-patil05" alt="Visitor Count" />
</div>
